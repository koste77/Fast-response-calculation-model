...
class FNO_Branch(nn.Module):
    def __init__(self, modes=16, width=64, hidden_dim=128):
        super(FNO_Branch, self).__init__()
...
class ME_BEP_PINO_Parallel(nn.Module):
    def __init__(self, m_sensors, modes=16, width=64, hidden_dim=128):
        super(ME_BEP_PINO_Parallel, self).__init__()
        self.m_sensors = m_sensors
        
        self.encoder_u = FNO_Branch(modes, width, hidden_dim)
        self.encoder_phi = FNO_Branch(modes, width, hidden_dim)
        self.encoder_M = FNO_Branch(modes, width, hidden_dim)
        self.encoder_Q = FNO_Branch(modes, width, hidden_dim)
    ...
    def forward(self, branch_input, x_batch):
        ...
        feat_u = self.encoder_u(fno_in)
        feat_phi = self.encoder_phi(fno_in)
        feat_M = self.encoder_M(fno_in)
        feat_Q = self.encoder_Q(fno_in)
        
        u_raw = self.decoder_u(feat_u)
        phi_raw = self.decoder_phi(feat_phi)
        M_raw = self.decoder_M(feat_M)
        Q_raw = self.decoder_Q(feat_Q)
...
model_path = "ME-BP-PFNO_Parallel_Model_fixed-fixed_Concentrated.pth"
logs_path = "ME-BP-PFNO_Parallel_Logs_fixed-fixed_Concentrated.pth"
...
def plot_standardized_training_history(logs):
    ...
    def stages(adam_key, lbfgs_key, label, adam_color=None, lbfgs_color=None):
        ...
        if adam.size:
            plt.semilogy(np.arange(adam.size), adam, color=adam_color, linewidth=1.8, linestyle='-', label=f'Adam {label}')
        if lbfgs.size:
            plt.semilogy(np.arange(adam.size, adam.size + lbfgs.size), lbfgs, color=lbfgs_color, linewidth=2.0, linestyle='--', label=f'L-BFGS {label}')

    plt.figure(figsize=(10, 6))
    stages('loss_history', 'lbfgs_loss_history', 'Total Physics Loss',
    ...
    for key, label in components:
        stages(f'loss_{key}_history', f'lbfgs_loss_{key}_history', label,
...
plt.ylabel('Loss (Log Scale)', fontsize=16)
plt.title('Training Convergence Curve (ME-BP-PFNO)', fontsize=16)    
plt.grid(True, which="both", ls="--", alpha=0.5)
...
print("\n" + "="*85)
print(" ME-BEP-PINO vs high precision Hermite FEM Quantitative evaluation report")
print("="*85)
