...
def compute_physics_loss_combined(model, branch_input, x_batch, q_y, EI_z, L, bc_type='fixed-fixed'):
    ...
    d2u_dx2 = compute_gradients_fd(du_dx, dx)
    d3u_dx3 = compute_gradients_fd(d2u_dx2, dx)
    d4u_dx4 = compute_gradients_fd(d3u_dx3, dx)

    m_sensors = branch_input.shape[1] - 2
    q_sensors = branch_input[:, 0:m_sensors]
    q_norm_abs = (torch.mean(torch.abs(q_sensors), dim=1).view(-1, 1, 1) + 1e-4) / 0.1

    loss_geo_norm = torch.mean(((phi_pred - du_dx) / (model.Phi_scale * q_norm_abs)) ** 2)
    loss_const_norm = torch.mean(((M_pred - EI_z * d2u_dx2) / (model.M_scale * q_norm_abs)) ** 2)
    loss_eq1_norm = torch.mean(((Q_pred - EI_z * d3u_dx3) / (model.Q_scale * q_norm_abs)) ** 2)
    loss_eq2_norm = torch.mean(((EI_z * d4u_dx4 - q_y) / (0.1 * q_norm_abs)) ** 2)
...
if __name__ == '__main__':
    ...
    model = HO_BEP_PINO_Combined(m_sensors=m_sensors, modes=16, width=64)
...
model_save_path = "HO-BEP-PINO_Model_fixed-fixed_Concentrated.pth"
...
logs_save_path = "HO-BEP-PINO_Logs_fixed-fixed_Concentrated.pth""
