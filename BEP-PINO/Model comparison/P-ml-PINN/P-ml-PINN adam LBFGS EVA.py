...
class PINN_model:
    def __init__(self, layers, X, q, X_uc, u_c, X_ac, a_c, X_kc, k_c,
                 EI=1.0, L=1.0, P_min=-5.0, P_max=-0.5, xp_min=0.2, xp_max=0.8):
    ...
    def net_u(self, X): return self.forward_pass(X, self.weights_u, self.biases_u, self.layers_u)
    def net_a(self, X): return self.forward_pass(X, self.weights_a, self.biases_a, self.layers_a)
    def net_k(self, X): return self.forward_pass(X, self.weights_k, self.biases_k, self.layers_k)
    def net_Q(self, X): return self.forward_pass(X, self.weights_Q, self.biases_Q, self.layers_Q)
...
if __name__ == '__main__':
    ...
    logs = np.load(save_prefix + '_logs.npz', allow_pickle=True)

    sigma_val = float(logs['sigma_val'])
    L_val = float(logs['L_val'])
    EI_val = float(logs['EI_val'])
    P_min_val, P_max_val = float(logs['P_min_val']), float(logs['P_max_val'])
    xp_min_val, xp_max_val = float(logs['xp_min_val']), float(logs['xp_max_val'])
    layers = logs['layers'].tolist()
    

    loss_log = logs['loss_log']
    loss_c_log = logs['loss_c_log']
    loss_f_log = logs['loss_f_log']
    loss_lbfgs_log = logs.get('loss_lbfgs_log', np.array([]))
    ...
    model = PINN_model(layers, X_fake, q_fake, X_fake, q_fake, X_fake, q_fake, X_fake, q_fake,
                       EI=EI_val, L=L_val, P_min=P_min_val, P_max=P_max_val, xp_min=xp_min_val, xp_max=xp_max_val)
    model.saver.restore(model.sess, save_prefix + '.ckpt')

    print(f"Loading: {save_prefix}.ckpt")

    len_adam = len(loss_log)
    len_lbfgs = len(loss_lbfgs_log)
    x_adam = np.arange(len_adam)
    x_lbfgs = np.arange(len_adam, len_adam + len_lbfgs)

    plt.figure(figsize=(10, 6))
    plt.semilogy(x_adam, loss_c_log, color="#7895C1", linewidth=1.5, label='Boundary Loss (Adam)')
    plt.semilogy(x_adam, loss_f_log, color="#EF8B67", linewidth=1.5, label='Physics Loss (Adam)')
    plt.semilogy(x_adam, loss_log, color="#C0321A", linewidth=1.8, label='Total Loss (Adam)')
    if len_lbfgs > 0:
        plt.semilogy(x_lbfgs, loss_lbfgs_log, color="#218D42", linewidth=2.0, linestyle='--', label='Total Loss (L-BFGS)')
        plt.axvline(x=len_adam, color='gray', linestyle=':', linewidth=1.5)
        ax = plt.gca()
    ...
    plt.title('Parametric PINN Training Loss: Boundary + Physics', fontsize=16)
    ...
    plt.figure(figsize=(10, 6))
    plt.semilogy(x_adam, logs['loss_Q_q_log'], linewidth=1.5, label=r'$dQ/dx+q$')
    plt.semilogy(x_adam, logs['loss_M_Q_log'], linewidth=1.5, label=r'$dM/dx-Q$')
    plt.semilogy(x_adam, logs['loss_a_k_log'], linewidth=1.5, label=r'$d\phi/dx+k$')
    plt.semilogy(x_adam, logs['loss_u_a_log'], linewidth=1.5, label=r'$du/dx-\phi$')
    plt.xlabel('Adam Epochs', fontsize=16)
    plt.ylabel('Residual Loss', fontsize=16)
    plt.title('Individual Physics Residuals (Adam Phase)', fontsize=16)
    ...
    fig_k, ax_k = plt.subplots(figsize=(10, 3))
    ...
    for i, (P_val, xp_val) in enumerate(test_loads):
        ...
        u_pred_np = model.predict_u(x_np, P_val, xp_val).reshape(-1)
        phi_pred_np = model.predict_a(x_np, P_val, xp_val).reshape(-1)
        k_pred_np = model.predict_k(x_np, P_val, xp_val).reshape(-1)
        M_pred_np = model.predict_M(x_np, P_val, xp_val).reshape(-1)
        Q_pred_np = model.predict_Q(x_np, P_val, xp_val).reshape(-1)
    ...
    print("\n" + "="*85)
    print("🚀 Parametric PINN vs High-Res FEM Quantitative evaluation report on structural response depth")
    print("="*85)
    ...
    print("\n" + "=" * 85)
    print(" Final Training Loss Summary (Parametric PINN: Adam + L-BFGS-B)")
    print("=" * 85)
    
    adam_final_loss = loss_log[-1]
    adam_final_c_loss = loss_c_log[-1]
    adam_final_f_loss = loss_f_log[-1]
    lbfgs_final_loss = loss_lbfgs_log[-1] if len(loss_lbfgs_log) > 0 else np.nan

    print(f"{'Phase':^22} | {'Total Loss':^18} | {'Boundary Loss':^18} | {'Physics Loss':^18}")
    print("-" * 85)
    print(f"{'Adam Final':^22} | {adam_final_loss:>18.6e} | {adam_final_c_loss:>18.6e} | {adam_final_f_loss:>18.6e}")
    print(f"{'L-BFGS Final':^22} | {lbfgs_final_loss:>18.6e} | {'-':^18} | {'-':^18}")
    print("=" * 85)
