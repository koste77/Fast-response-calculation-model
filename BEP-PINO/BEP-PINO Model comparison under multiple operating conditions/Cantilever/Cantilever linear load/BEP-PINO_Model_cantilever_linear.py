...
TARGET_RELATIVE_DIR = Path("BEP-PINO Model comparison under multiple operating conditions/Cantilever/Cantilever linear load")
MODEL_SAVE_PATH = "BEP-PINO_Model_Linear_cantilever.pth"
LOGS_SAVE_PATH = "BEP-PINO_Logs_Linear_cantilever.pth"
BC_TYPE = "cantilever"
LOAD_KIND = "linear"
...
Q_MIN_VAL, Q_MAX_VAL = -0.3, -0.001
LHS_DIM = 2


def build_load_batch(lhs_samples, batch_size, m_sensors, n_domain, L_val, EI_val):
    q0_np = Q_MIN_VAL + lhs_samples[:, 0:1] * (Q_MAX_VAL - Q_MIN_VAL)
    qL_np = Q_MIN_VAL + lhs_samples[:, 1:2] * (Q_MAX_VAL - Q_MIN_VAL)
    x_sensors = np.linspace(0.0, L_val, m_sensors)
    q_sensor_np = q0_np + (qL_np - q0_np) * x_sensors / L_val
    ...
    q0_tensor = torch.tensor(q0_np, dtype=torch.float64).unsqueeze(2)
    qL_tensor = torch.tensor(qL_np, dtype=torch.float64).unsqueeze(2)
    q_domain = q0_tensor + (qL_tensor - q0_tensor) * x_batch / L_val
    return branch_input, x_batch, q_domain


def load_metadata():
    return {"q_min_val": Q_MIN_VAL, "q_max_val": Q_MAX_VAL}
...
if __name__ == "__main__":
    ...
    print("================ Adam Training: Linear Distributed Load / Cantilever ================")
