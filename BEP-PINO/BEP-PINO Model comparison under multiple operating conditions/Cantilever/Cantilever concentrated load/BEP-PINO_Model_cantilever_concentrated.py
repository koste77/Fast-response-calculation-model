...
TARGET_RELATIVE_DIR = Path("BEP-PINO Model comparison under multiple operating conditions/Cantilever/Cantilever Concentrated load")
MODEL_SAVE_PATH = "BEP-PINO_Model_Concentrated_cantilever.pth"
LOGS_SAVE_PATH = "BEP-PINO_Logs_Concentrated_cantilever.pth"
BC_TYPE = "cantilever"
...
def build_load_batch(lhs_samples, batch_size, m_sensors, n_domain, L_val, EI_val):
    ...
    q_sensor_np = P_np * np.exp(-((x_sensors - xp_np)**2) / (2.0 * SIGMA_VAL**2))
    ...
    q_domain = P_tensor * torch.exp(-((x_batch - xp_tensor)**2) / (2.0 * SIGMA_VAL**2))
    return branch_input, x_batch, q_domain


def load_metadata():
    return {
        "P_min_val": P_MIN_VAL, "P_max_val": P_MAX_VAL,
        "xp_min_val": XP_MIN_VAL, "xp_max_val": XP_MAX_VAL,
        "sigma": SIGMA_VAL,
    }
...
if __name__ == "__main__":
    ...
    print("================ Adam Training: Gaussian Localized Load / Cantilever ================")
