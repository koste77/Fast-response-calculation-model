...
TARGET_RELATIVE_DIR = Path("BEP-PINO Model comparison under multiple operating conditions/Fixed-fixed/Fixed-fixed quadratic load")
MODEL_SAVE_PATH = "BEP-PINO_Model_Quadratic_2Param_fixed-fixed.pth"
LOGS_SAVE_PATH = "BEP-PINO_Logs_Quadratic_2Param_fixed-fixed.pth"
BC_TYPE = "fixed-fixed"
LOAD_KIND = "quadratic"
...
class FNO_Combined(nn.Module):
    ...
    def predict_with_ansatz(self, branch_input, x_batch, L, bc_type=BC_TYPE):
        ...
        input_sensors = branch_input[:, :self.m_sensors]
        response_scale = torch.mean(input_sensors, dim=1, keepdim=True).unsqueeze(1) / 0.1
...
def compute_physics_loss_combined(model, branch_input, x_batch, q_y, EI_z, L, bc_type=BC_TYPE):
    ...
    input_sensors = branch_input[:, :model.m_sensors]
    input_norm = (torch.mean(torch.abs(input_sensors), dim=1).view(-1, 1, 1) + 1e-4) / 0.1
    eq2_scale = 0.1 * input_norm
...
QE_MIN_VAL, QE_MAX_VAL = -0.3, -0.001
DQ_MIN_VAL, DQ_MAX_VAL = 0.01, 0.15
LHS_DIM = 2


def build_load_batch(lhs_samples, batch_size, m_sensors, n_domain, L_val, EI_val):
    q_end_np = QE_MIN_VAL + lhs_samples[:, 0:1] * (QE_MAX_VAL - QE_MIN_VAL)
    dq_np = DQ_MIN_VAL + lhs_samples[:, 1:2] * (DQ_MAX_VAL - DQ_MIN_VAL)
    c0_np = q_end_np
    c2_np = 4.0 * dq_np / (L_val**2)
    c1_np = -4.0 * dq_np / L_val
    x_sensors = np.linspace(0.0, L_val, m_sensors)
    q_sensor_np = c2_np * x_sensors**2 + c1_np * x_sensors + c0_np
    branch_input = torch.cat([
        torch.tensor(q_sensor_np, dtype=torch.float64),
    ...
    c0_tensor = torch.tensor(c0_np, dtype=torch.float64).unsqueeze(2)
    c1_tensor = torch.tensor(c1_np, dtype=torch.float64).unsqueeze(2)
    c2_tensor = torch.tensor(c2_np, dtype=torch.float64).unsqueeze(2)
    q_domain = c2_tensor * x_batch**2 + c1_tensor * x_batch + c0_tensor
    return branch_input, x_batch, q_domain


def load_metadata():
    return {
        "qe_min_val": QE_MIN_VAL, "qe_max_val": QE_MAX_VAL,
        "dq_min_val": DQ_MIN_VAL, "dq_max_val": DQ_MAX_VAL,
        "load_parameterization": "symmetric_quadratic_2param_qe_dq",
    }
...
if __name__ == "__main__":
    ...
    print("================ Adam Training: Symmetric Quadratic Load / Fixed-Fixed ================")
