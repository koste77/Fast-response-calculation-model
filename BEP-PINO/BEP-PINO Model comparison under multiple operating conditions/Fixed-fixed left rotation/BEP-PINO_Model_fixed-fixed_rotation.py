...
TARGET_RELATIVE_DIR = Path("BEP-PINO Model comparison under multiple operating conditions/Fixed-fixed left rotation")
MODEL_SAVE_PATH = "BEP-PINO_Model_Rotation_fixed-fixed.pth"
LOGS_SAVE_PATH = "BEP-PINO_Logs_Rotation_fixed-fixed.pth"
...
class FNO_Combined(nn.Module):
    ...
    def predict_with_ansatz(self, branch_input, x_batch, L, bc_type=BC_TYPE):
        ...
        xi = x_batch / L
        lifting_u = case_value * L * (xi - 2.0 * xi**2 + xi**3)
        lifting_phi = case_value * (1.0 - 4.0 * xi + 3.0 * xi**2)
...
THETA_MIN_VAL, THETA_MAX_VAL = -0.1, 0.1
LHS_DIM = 1


def build_load_batch(lhs_samples, batch_size, m_sensors, n_domain, L_val, EI_val):
    theta_np = THETA_MIN_VAL + lhs_samples[:, 0:1] * (THETA_MAX_VAL - THETA_MIN_VAL)
    sensor_np = np.repeat(theta_np, m_sensors, axis=1)
...
def load_metadata():
    return {"theta_min_val": THETA_MIN_VAL, "theta_max_val": THETA_MAX_VAL, "boundary_type": BC_TYPE}
...
if __name__ == "__main__":
    ...
    print("================ Adam Training: Support Rotation Input / Fixed-Fixed Rotation ================")
